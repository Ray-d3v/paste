...
- Install once per clone: `powershell -ExecutionPolicy Bypass -File tools/scripts/install-git-hooks.ps1`
- On every `git push`, the hook runs `tools/scripts/build-local-exe.ps1 -NoLaunch`.
- If EXE generation fails, push is blocked.
- Emergency bypass (use sparingly): `git push --no-verify`

Push-success auto-launch flow:
- Use `powershell -ExecutionPolicy Bypass -File tools/scripts/push-and-launch.ps1 <git-push-args>`.
